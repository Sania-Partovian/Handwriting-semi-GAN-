...
class Dataset:
    def __init__(self,num_labeled):
        ...
        # Preprocess labels and add a dimention (3D to 4D (60000,28,28,1))
        def preprocess_img(x):
            x = (x.astype(np.float)-127.5) / 127.5   # Normalize between -1 and 1
            x = np.expand_dims(x,axis=3)       # To add a dimention
            return x

        # Preprocess labels and add a dimention (1D to 2D (60000,1))
    ...
    # This function reads some random labeled data according to batch-size from 0 to 100
    ...
    # This function reads some random unlabeled data according to batch-size from 100 to 60000
...
# Noise vector size
zdim=100
# Number of output classes (i.e. digits)
num_classes = 10
...
def train(iterations,batch_size,interval):
    ...
    for iteration in range(iterations):
        ...
        if (iteration+1) % interval == 0:
            ...
            # Print loss with four decimal points accuracy
...
# Print accuracy with two decimal points accuracy
